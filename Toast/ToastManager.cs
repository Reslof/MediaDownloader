...
namespace DesktopToast
{
    ...
    public class ToastManager
    {
        ...
        public static async Task<ToastResult> ShowAsync(ToastRequest request, int expire)
        {
            if (request == null)
            {
            ...
            XmlDocument document = PrepareToastDocument(request);
            if (document == null)
            {
                return ToastResult.Invalid;
            }

            return await ShowBaseAsync(document, request.AppId, expire);
        }

        /// <summary>
        /// Show toast without response
        /// </summary>
        ...
        public static async void Show(ToastRequest request, int expire)
        {
            ...
            ShowBase(document, request.AppId, expire);
        }
        ...
        /// <param name="expire"></param>
        /// <returns>Result of showing a toast</returns>
        public static async Task<ToastResult> ShowAsync(string requestJson, int expire)
        {
            ...
            return await ShowAsync(request, expire);
        }
        ...
        /// <param name="expire"></param>
        /// <returns>Result of showing a toast</returns>
        public static async Task<ToastResult> ShowAsync(XmlDocument document, string appId, int expire)
        {
            if (document == null)
            {
            ...
            return await ShowBaseAsync(document, appId, expire);
        }
        ...
        private static XmlDocument ComposeVisualForWin10(ToastRequest request)
        {
            ...
            {
                XmlElement toastTitle = document.CreateElement("text");
            ...
            {
                XmlElement toastBody = document.CreateElement("text");
            ...
            {
                XmlElement appLogo = document.CreateElement("image");
        ...
        {
            ToastTemplateType templateType = GetTemplateType(request);

            // Get a toast template.
            XmlDocument document = ToastNotificationManager.GetTemplateContent(templateType);
        ...
        {
            AudioOption option = CheckAudio(request.ToastAudio);
            if (option == AudioOption.Long)
            {
                document.DocumentElement.SetAttribute("duration", "long");
            }

            XmlElement audioElement = document.CreateElement("audio");
        ...
        {
            string shortcutFilePath = Path.Combine(
        ...
        /// <param name="expire"></param>
        /// <returns>Result of showing a toast</returns>
        private static async Task<ToastResult> ShowBaseAsync(XmlDocument document, string appId, int expire)
        {
            ...
            ToastNotification toast = new ToastNotification(document)
            {
                ...
                ExpirationTime = DateTimeOffset.Now.AddSeconds(expire)
            };

            TaskCompletionSource<ToastResult> tcs = new TaskCompletionSource<ToastResult>();
            ...
            // Wait for the result.
            ToastResult result = await tcs.Task;
        ...
        private static void ShowBase(XmlDocument document, string appId, int expire)
        {
            // Create a toast and prepare to handle toast events.
            ToastNotification toast = new ToastNotification(document)
            {
            ...
            // Wait for the result.
            Task<ToastResult> result = tcs.Task;
